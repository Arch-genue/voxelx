...
void ShowInspector(ECSManager& ecsmanager, int& selectedIndex) {
    ...
    // Список объектов
    size_t size = ecsmanager.get_entities().get_id();
    ImGui::Text("GameObjects: %d", size);
    ...
    if (selectedIndex >= 0 && selectedIndex < (int)size) {
        // GameObject* obj = objects[selectedIndex];
        // glm::vec3 tempPos = obj->getPosition();
        if (ecsmanager.hasComponent<Transform>(selectedIndex)) {
            glm::vec3 tempPos = ecsmanager.getComponent<Transform>(selectedIndex).position;
            if (ImGui::DragFloat3("Position", &tempPos[0], 0.1f)) {
                ecsmanager.getComponent<Transform>(selectedIndex).position = tempPos;
            }
        }
